...
function queryRoute(routes, pathname) {
  ...
  const query = utils.customFind(routes, val => {
    const {
      path,
      exact
    } = val.attrs;
...
function BrowserRouter(props) {
  const Comp = reactivity.ref(props.loading);
  let attrs = {};
  ...
  return vdom.h(vdom.Fragment, null, () => Comp.value && vdom.h(Comp.value, {
    ...attrs
  }));
}
...
function resultReplace(replaceStr, Comp, attrs) {
  const string = currentApp.renderToString(vdom.h(Comp, {
    ...attrs
  }));
 ...
 * @returns
 */
async function ssrOutlet(content, url, template) {
