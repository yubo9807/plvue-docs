...
function BrowserRouter(props) {
  ...
  function routeChange(path) {
    ...
    // 全局守卫
    if (beforeEach) {
      beforeEach(reactivity.toRaw(currentRoute), backupRoute, () => {
    ...
    if (query.beforeEnter) {
      query.beforeEnter(reactivity.toRaw(currentRoute), backupRoute, () => {
        backupRoute = utils.deepClone(currentRoute);
        path === currentRoute.path ? next() : routeChange(currentRoute.path);
      });
      return;
...
function StaticRouter(props) {
  ...
  if (lock) return vdom.h(vdom.Fragment, null);
...
function Link(props) {
  if (!utils.isString(props.to)) {
    props.to = splicingUrl(props.to);
  }
...
async function ssrOutlet(App, url, template) {
  return new Promise(resolve => {
    ...
    const unWatch = reactivity.watch(() => stack.length, value => {
      if (value === 0) {
